...
def HasArchive(Archive, ArchiveName):
	try :
		BeforeFlush("Has Channel")
		Flush(str(Archive["has_channel"]), W, W, 0.05, 1)
		SaveAsTextFile("[~] Has Channel : "+str(Archive["has_channel"]), ArchiveName)
	except :
		BeforeFlush("Has Channel")
		Flush("None", W, W, 0.05, 1)
		SaveAsTextFile("[~] Has Channel : "+"None", ArchiveName)
	print ("")
...
def HasOwnArchive(Archive, ArchiveName): 
	try: 
		BeforeFlush('Has own archived content')
		Flush(str(os.POSIX_FADV_SEQUENTIAL), W, W, 0.05, 1)
		SaveAsTextFile("[~] Has Own Archived Content: " + str(Archive), ArchiveName)
	except: 
		BeforeFlush('Has Private Content')
		Flush("None", W, W, 0.05, 1)
		SaveAsTextFile("[~] Has Private Content : "+"None", ArchiveName)
	print("")
